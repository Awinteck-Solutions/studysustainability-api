    ...
    static async signup(req: Request, res: Response) {
       ...
       //generate OTP code 
        ...
        admin.save().then((response) => {
            const token = encrypt.generateToken({ _id: response._id, email: response.email, firstname: response.firstname, lastname:response.lastname, role: response.role });
           
            return res.status(201).json({
                message: 'New admin registered',
                response: {...formatAdminResponse(response,token), id:response._id}
            });
        }).catch((error) => {
    ...
    static async login(req: Request, res: Response) {
        try {
            ...
            .then(async (response) => {
                ...
                let isPasswordCorrect = await encrypt.comparepassword(hashPassword, password);
                console.log('isPasswordCorrect :>> ', isPasswordCorrect);
                if (isPasswordCorrect) {
                    
                    const token = encrypt.generateToken({ id: response._id, email: response.email, firstname: response.firstname, lastname:response.lastname, role: response.role });
                    return res.json({
                        message: 'Login success',
                        response: {...formatAdminResponse(response,token), _id:response._id}
                    })
                } else {
                    res.status(404).json({
